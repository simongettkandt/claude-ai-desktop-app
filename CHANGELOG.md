...
- **A permanent pointer to Anthropic's official Linux app.** Anthropic ships one since 2026-06-30, so the app says so instead of staying quiet about it: an "Official app" pill in the tab bar, an "Official Claude app" entry in the hamburger menu, and the first What's New slide. All three open the same dialog with requirements, scope and the two gaps the official build leaves (no Ubuntu 20.04, no snap), and link to Anthropic's install guide. Deliberately no apt commands in the app: repository path, key fingerprint and distro list live in their docs and would go stale here.
- A floating thank-you badge on the page, dismissable, remembered in `localStorage` (`inject/thanks-badge.js`). It skips Cloudflare interstitials so it never covers the verify banner.
...
- **The Design window would have been dead in every packaged build.** `preload-design.js` was missing from the electron-builder `files` list, so it never entered the asar. Without it `window.designAPI` is undefined: theme clicks only moved the local preview, and since the window is frameless with no menu, all three close paths (Done, titlebar X, Escape) went through the same missing bridge, leaving it closable only through the window manager. Verified by listing the built asar, which held nine of ten preloads.
- **The Neon preview showed the wrong ring colour.** The style cards only carried the button pair, and the click handler wrote it into the composer ring, which takes the lighter decorative pair. Clicking Neon showed `#1B54BE`, reopening the window showed `#2F7FFF`. The cards now carry both pairs, matching what `card()` does on first render.
- The menu action that opens the Design window was named `design-toggle`, the same string as the IPC channel that cycles the accent style. It is `design-open` now.

### Renamed

- **Fully separated from Anthropic's own Linux package.** Their deb owns `/usr/bin/claude-desktop`, `com.anthropic.Claude.desktop` and the icon name `claude-desktop`. This app used the same icon name and the same `.desktop` filename, both derived from `executableName`. Measured consequence: `setDesignStyle()` writes `~/.local/share/icons/hicolor/*/apps/claude-desktop.png` on every style switch, and the user icon theme outranks `/usr/share/icons`, so a style switch here would have recoloured their menu icon. `executableName` is `desktop-for-claude` now, which moves binary, `.desktop` filename and icon name out of the way; the snap launcher path and the icon write path follow. `StartupWMClass` stays `claude-desktop` because Electron derives WM_CLASS from `app.getName()`, not from the binary, verified with `wmctrl -lx`. The Flatpak entry no longer claims `x-scheme-handler/claude`, which belongs to Anthropic. The `.desktop` self-heal now checks the old filename too, so installs from before the rename keep working.
- Category is `Development;Utility` instead of `Utility`, so the app also shows up under Development in application menus.
- Bug reports go to `simonlinuxcraft@pm.me`. The old Gmail address is gone from the app and the store listing.
- **The app is called Desktop for Claude.** Anthropic ships its own Claude app for Linux since 2026-06-30, so a product of the same name is no longer a description but a collision. The app now refers to Claude instead of naming itself that: window titles, tray tooltip, About window, app menu, autostart entry, bug report metadata and the Snap store title (`title:`, which leaves the snap `name` and with it the installed base untouched). `appId` moves from `com.claude.desktop`, a domain that is not ours, to `de.gettkandt.desktopforclaude`. `executableName` is pinned to `claude-desktop` so the binary name, the `.desktop` file and the AppImage artifact name stay as they are: the update path and desktop integration do not move. The userData directory is unaffected, it derives from the root `name` field, so chats, cookies and settings stay in place.
